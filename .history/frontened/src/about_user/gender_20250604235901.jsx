			...
			<div className="w-full sm:max-w-xl md:max-w-3xl">
				<div className="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
					...
					<div className="flex grid grid-cols-2 gap-5 justify-between py-5 items-center">
						...
						{gen.map((i, index) => (
							<div key={index}>
								<button className={`w-full p-2 text-white text-2xl rounded-xl bg-gradient-to-br ${!gender === i.name ? 'from-zinc-100 to-gray-400' : 'from-indigo-500 to-purple-500'} cursor-pointer transition hover:px-4`} 
								onClick={() => {
									setGender(i.name);
									setImage(i.image)
								}}
