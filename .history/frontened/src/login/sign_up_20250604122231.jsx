...
function Sign_up({ setOtp, setEmail }) {

  const [notification, setNotification] = useState({});
  const [show_notify, setShow_notify] = useState(false);
  ...
  const handle_login = async function(){
    if(data.username.length > 0 && data.email.length > 0 && data.password.length > 0){
      ...
      setNotification({Error : "Fill the details first!"})
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center p-4">
      <div className="w-full max-w-6xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row">
          ...
          {show_notify && 
           <Notify notification={notification} setNotification={setNotification} setShow_notify={setShow_notify}tify/>
          }
          ...
          <div className="w-full md:w-2/5 bg-gradient-to-br from-indigo-600 to-purple-700 p-8 md:p-12 flex flex-col justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-3xl font-bold text-white">FitPanda</h1>
            </div>
